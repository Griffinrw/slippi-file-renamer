...
def rename_slippi_files(directory):
    ...
    for filename in glob.iglob(directory + '**/**', recursive=True):
        if os.path.isfile(filename):
            new_filename = os.path.dirname(filename) + '\\' + get_info(filename) + '.slp'
            #actual_path = os.path.dirname(filename)
...
def recursion_test(root_dir):
    for filename in glob.iglob(root_dir + '**/**', recursive=True):
        if os.path.isfile(filename):
            print('is file:', filename)
        else:
            #print('isn\'t file:', filename)
            pass
...
if __name__ == '__main__':
    ...
    #recursion_test('C:\\Users\\talen\\Desktop\\testfolder')
